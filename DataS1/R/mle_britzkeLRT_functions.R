...
# # log lhood fun
# lhood_twospp <- function(theta, phi_mat, n_vec){
#   n1 <- n_vec[1]
#   N <- sum(n_vec)
#   p <- theta*phi_mat[1,1] + (1-theta)*phi_mat[1,2]
...
# MEE fake data simulation function -- uses numerical optimization
# could do with analytic MLE too - see below
...
#KMI edit to not spit out so much output
...
occu_compare_simple <- function(mle_output, n_visit = 3,
  ...
  # psi_rem <- fit_remove@estimates@estimates$state@estimates
  # se_psi_rem <- sqrt(fit_remove@estimates@estimates$state@covMat)
  # psi_naive <- fit_naive@estimates@estimates$state@estimates
  # se_psi_naive <- sqrt(fit_naive@estimates@estimates$state@covMat)
...
# Britzke MLE LRT function for one site (aggregated over visits), calls lhood
